...
class progWidgetAdvanced(qt.QWidget):
    def __init__(self, parent=None):
        super(progWidgetAdvanced, self).__init__(parent)
...
class profileWidgetUi(qt.QWidget):
    def __init__(self, name: str, parent=None):
        super(profileWidgetUi, self).__init__(parent=parent)
        self.name = name

        # Create the layout for the table and buttons
        layer2 = qt.QHBoxLayout()
        layer3 = qt.QVBoxLayout()
        ...
        self.buttonARM = qt.QPushButton('ARM')
        self.buttonADD = qt.QPushButton('+')
        self.buttonSAVE = qt.QPushButton('SAVE')
        self.buttonLOAD = qt.QPushButton('LOAD')
        self.buttonDEL1s = qt.QPushButton('-')
        self.typecbox = qt.QComboBox()
        self.typecbox.addItems(["°C", "V"])

        # Create labels for the table columns
        typelabel = qt.QLabel('Value type:')
        labeltime = qt.QLabel()
        labelval = qt.QLabel()
        ...
        layer2.addWidget(self.buttonADD, 0)
        layer2.addWidget(self.buttonDEL1s, 0)
        layer2.addWidget(self.buttonSAVE, 0)
        layer2.addWidget(self.buttonLOAD, 0)
        layer2.addWidget(typelabel, 0)
        layer2.addWidget(self.typecbox, 0)
        layer2.addWidget(self.buttonARM, 0)

        # Set the size of the add and delete buttons
        self.buttonADD.setFixedSize(60, 25)
        self.buttonDEL1s.setFixedSize(60, 25)
...
class profileWidget(profileWidgetUi):
    def __init__(self, parent=None):
        super(profileWidget, self).__init__(parent)

        # Connect button signals to their corresponding functions
        self.buttonADD.clicked.connect(lambda: self.add_layer(self.tableWidget.rowCount()))
        self.buttonDEL1s.clicked.connect(lambda: self.mass_del())
        self.buttonARM.clicked.connect(lambda: self.parent().parent().parent().get_main_dict())
        self.buttonSAVE.clicked.connect(lambda: self.saving_procedure())
        self.buttonLOAD.clicked.connect(lambda: self.table_from_file())
    ...
    def saving_procedure(self):
        ...
        file_path, _ = qt.QFileDialog.getSaveFileName(self, "New File", "", "JSON Files (*.json)", options=options)
    ...
    def table_from_file(self):
        ...
        options = qt.QFileDialog.Options()
        options |= qt.QFileDialog.DontUseNativeDialog  # add the QFileDialog.DontUseNativeDialog option to options
        # Open file dialog and get file path
        file_path, _ = qt.QFileDialog.getOpenFileName(self, "Open File", "", "JSON Files (*.json)",
        ...
        for key in rows_data.keys():
            ...
            buttrow = qt.QPushButton(row_data['mode_type'])
            ...
            flabel = qt.QLabel("Ampl: " if mode_type == "Sine Segment" else "From:")
            spineditval1 = qt.QLabel(str(val1))
            labelval1 = qt.QLabel(typeval1 if mode_type != "Sine Segment" else "")
            ...
            if mode_type == "Ramp":
                tLabel = qt.QLabel("To: ")
                spineditval2 = qt.QLabel(val2)
                labelval2 = qt.QLabel(typeval2)
                hlayer1.addWidget(tLabel)
                hlayer1.addWidget(spineditval2)
                hlayer1.addWidget(labelval2)
                wid1.setLayout(hlayer1)
            if mode_type == "Sine Segment":
                tLabel = qt.QLabel("Freq: ")
                oLabel = qt.QLabel("Offs: ")
                spineditval2 = qt.QLabel(val2)
                spineditval3 = qt.QLabel(val3)
                labelval2 = qt.QLabel()
                hlayer1.addWidget(tLabel)
                hlayer1.addWidget(spineditval2)
            ...
            spineditduration = qt.QLabel(
                "Duration: " if mode_type == "Isotherm" or mode_type == "Sine Segment" else "Rate: ")
            labeltime = qt.QLabel(typetime)
            labelvaltime = qt.QLabel(time)
    ...
    def add_layer(self, x: int):
        dialog = progSegmentDialog(self.typecbox.currentText(), False)
        if dialog.exec_() == qt.QDialog.Accepted:
            ...
            # Create and set the button widget
            buttrow = qt.QPushButton(mode_type)
            buttrow.setObjectName("but" + str(row_number))
            buttrow.clicked.connect(lambda checked, x=row_number: self.add_layer(x))
            self.tableWidget.setCellWidget(row_number, 0, buttrow)
            ...
            spineditduration = qt.QLabel(
                "Duration: " if mode_type == "Isotherm" or mode_type == "Sine Segment" else "Rate: ")
            labeltime = qt.QLabel(self.typecbox.currentText() + "/" + typetime if mode_type == "Ramp" else typetime)
            labelvaltime = qt.QLabel(time)
            hlayer3 = qt.QHBoxLayout()
            hlayer3.addWidget(spineditduration)
            hlayer3.addWidget(labelvaltime)
