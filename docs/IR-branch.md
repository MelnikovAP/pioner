...
| **`Rhtr` formula**               | `Rhtr = (Uhtr_mV - Uref_mV + uhtr0) * uhtr1 / Ih`. With identity calibration this yields mV/A = mΩ; with production `ihtr1 ≈ 1/Rshunt` and Uhtr_mV converted from V via `*1000`, the implicit assumption is `uhtr0` in mV. | Documented: keeps both numerator and denominator in V/A → Ω directly (see `back/modes.py:240`-280 comment). IR formula is consistent only if you remember `uhtr0` is in mV. |
