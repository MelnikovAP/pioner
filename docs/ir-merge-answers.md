...
гипотетической SI-калибровке `ihtr1 ≈ 1/R_shunt` это дало бы **напряжение на
шунте**, а не на нагревателе (production использует identity `ihtr0=0,
ihtr1=1`, см. P0-3; SI -- будущее, P2-21). Но кладётся это на AO наружу.
...
- Pb выше `safe_voltage` — это фича для возможных в будущем
  высокотемпературных чипов.
...
- Это алгоритм демодуляции, основанный на коде Бондаря.
...
**Ответ IR:** код не отлажен, есть сомнения в его необходимости.
Линейная интерполяция была бы лучше — текущая ступенчатая реализация
это упрощение.
...
**Ответ IR:** обе версии рассматривались в работе с железом, однако
pandas DataFrame оказался стабильнее (возможно, просто не получилось
оптимизировать numpy) и более удобен в работе. (Почему закомментированный
numpy-блок не удалён — без явного ответа.)
...
**Ответ IR:** тут и первое, и второе. Критичные сценарии для regression
testing — длительности экспериментов и стабильность работы в течение
этого времени; нужно убрать лаги при длительных экспериментах (возможно
делением профиля на дополнительные подсегменты данных).
...
пары owner'ов гарантированно несовместимы — уже не вспомнить.
...
**Ответ IR:**
- `ScanOption.DEFAULTIO` починил проблему: «да, это было какое-то
  магическое действие, которое резко всё починило».
- `AiInputMode.DIFFERENTIAL`: «нет, дифференциального режима для платы
  сбора данных нет» — прямо подтверждает постмортем
  [2026-05-23-differential-vs-defaultio.md](../postmortem/2026-05-23-differential-vs-defaultio.md)
  (USB-2637 поддерживает только SINGLE_ENDED, фикс был `ScanOption.DEFAULTIO`).
...
- ответ на **C2** — `AOStreamSHGenerator` со ступенчатыми параметрами
  ...
  заменить на линейную интерполяцию либо не портировать до явной нужды;
- ответ на **F2** — критичный regression-сценарий: длительные эксперименты
  и стабильность по времени (лаги при длинных профилях — кандидат на
  деление на подсегменты);
- ответ на **H2** — поможет переименовать поле в конфиге без неоднозначности.

## Что осталось без явного ответа

После последнего раунда уточнений остаётся одна явно неотвеченная
позиция:

- **D1** — почему закомментированный numpy-блок `apply_fh_cal` не удалён
  из IR-ветки. Не блокер, можно просто удалить блок при порте.

Всё остальное закрыто либо прямым ответом, либо «не помню» (G2 — по
существу тоже ответ: пар несовместимых owner'ов IR не помнит).
