...
@dataclass # turns a simple class of attributes into a data container by auto-generating boilerplate like __init__, __repr__, and comparisons
...
try:
    ...
    DAQ_AVAILABLE = False
    logger.warning(f"Real uldaq not available ({e}) - using SECURE MOCK hardware for development/testing")
    ...
    class MockAiInfo:
        ...
        def get_num_chans_by_mode(self, mode):
            ...
            if mode not in [AiInputMode.SINGLE_ENDED, AiInputMode.DIFFERENTIAL]:
                ...
                logger.warning(f"Mock AI device: Invalid mode {safe_mode}, defaulting to 8 channels")
                return 8
    ...
    class MockAiDevice:
        ...
        def disconnect(self):
            if not security_manager.validate_access("ai_disconnect", self._security_context):
                raise RuntimeError("Access denied: security validation failed")
        ...
        def get_scan_status(self):
            ...
            if not security_manager.validate_access("ai_get_status", self._security_context):
                raise RuntimeError("Access denied: security validation failed")
        ...
        def get_buffer(self):
            ...
            if not security_manager.validate_access("ai_get_buffer", self._security_context):
                raise RuntimeError("Access denied: security validation failed")
        ...
        def a_in_scan(self, low_channel, high_channel, input_mode, analog_range, 
            ...
            if not security_manager.validate_access("ai_scan_start", self._security_context):
                raise RuntimeError("Access denied: security validation failed")
            ...
            if low_channel < 0 or high_channel > MAX_CHANNELS - 1:
                raise ValueError(f"Channel range must be 0-{MAX_CHANNELS - 1} for mock device")
            ...
            if not isinstance(input_mode, int) or input_mode not in [AiInputMode.SINGLE_ENDED, AiInputMode.DIFFERENTIAL]:
            ...
            try:
                expected_buffer_size = (high_channel - low_channel + 1) * samples_per_channel
            ...
            if len(buffer) != expected_buffer_size:
                raise ValueError(f"Buffer size {len(buffer)} doesn't match expected size {expected_buffer_size}")
    ...
    class MockAoDevice:
        ...
        def disconnect(self):
            if not security_manager.validate_access("ao_disconnect", self._security_context):
                raise RuntimeError("Access denied: security validation failed")
        ...
        def get_scan_status(self):
            """Get scan and transfer status with validation."""
            if not security_manager.validate_access("ao_get_status", self._security_context):
                raise RuntimeError("Access denied: security validation failed")
        ...
        def a_out_scan(self, low_channel, high_channel, analog_range, 
            ...
            if not security_manager.validate_access("ao_scan_start", self._security_context):
                raise RuntimeError("Access denied: security validation failed")
            
            # Comprehensive input validation
            if not isinstance(low_channel, int) or not isinstance(high_channel, int):
                raise ValueError("Channel numbers must be integers")
            if low_channel < 0 or high_channel > 3:  # AO devices typically have fewer channels
            ...
            if len(ao_buffer) != expected_buffer_size:
                raise ValueError(f"Buffer size {len(ao_buffer)} doesn't match expected size {expected_buffer_size}")
    ...
    class MockTransferStatus:
        ...
        def __init__(self, current_scan_count: int, current_total_count: int, current_index: int):
    ...
    def get_daq_device_inventory(interface_type, max_devices):
        ...
        if not isinstance(interface_type, int) or interface_type not in [1, 2, 4, 7]:
            raise ValueError(f"Invalid interface_type {interface_type}. Must be one of: 1 (USB), 2 (BLUETOOTH), 4 (ETHERNET), 7 (ANY)")
    ...
    def create_float_buffer(channel_count, sample_rate):
        ...
        logger.info(f"Mock: Creating secure float buffer with {channel_count} channels, {sample_rate} samples")
