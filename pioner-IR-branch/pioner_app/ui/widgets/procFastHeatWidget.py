...
from pioner_app.ui.h_windows import *
...
class _ProcFastHeatPlotWindow(PlotWindow):
...
class ProcFastHeatResultPlot(qt.QWidget):
    def __init__(self, parent=None):
        ...
        self.resultPlot = _ProcFastHeatPlotWindow(self)
    ...
    def addCurve(self, x, y, **kwargs):
        ...
        self._curve_store.append({
            ...
            'is_fit': kwargs.pop('is_fit', False),
        })
...
class procFastHeatWidget(qt.QWidget):
    def __init__(self, parent=None):
        ...
        ## Graph space
        self.procFastHeatGraph = ProcFastHeatResultPlot(parent=self)
        ...
        self.fitSegmentButton = qt.QPushButton("Fit segment")
        lout_1.addWidget(self.fitSegmentButton)
        lout_1 = qt.QHBoxLayout()
        lout_0.addLayout(lout_1)
        lout_1.addStretch()
        ...
        self.fitSegmentButton.clicked.connect(self.fit_selected_segment)
    ...
    def _load_fast_heat_data_group(self, path):
        ...
        payload = {
            ...
            'temp': self._read_dataset_with_aliases(data_group, 'temp', 'Ttpl'),
            'Uref': self._read_dataset_with_aliases(data_group, 'Uref', 'Ref', 'Ihtr'),
            'Thtr': self._read_dataset_with_aliases(data_group, 'Thtr'),
        }
        data_group.close()
        return payload
    ...
    def display_rate_button_pressed(self):
        ...
        if self.expFilesTable.count()>1:
            ...
            buttonReply = YesCancelWindow(warning, self.window()).exec()
    ...
    def calculate_manual_button_pressed(self):
        ...
        if self.saveFileOption.checkState() == 2:         # 2 = checked
            ...
            dest_file = qt.QFileDialog.getSaveFileName(
                self,
    ...
    def fit_selected_segment(self):
        ...
        if not curves:
            ErrorWindow("No visible curve data inside the selected segment.", self.window())
            return
        ...
        for curve in curves:
            ...
            slope = coeffs[0]
            intercept = coeffs[1]
            legend = curve['legend'] or 'segment'
            fit_legend = f"fit {legend}: k={slope:.4g}, b={intercept:.4g}"
            self.procFastHeatGraph.addCurve(
                x,
                fit_y,
                legend=fit_legend,
                color=self.procFastHeatGraph.curveColors['fit'],
                linewidth=2,
                linestyle='--',
                is_fit=True,
            )
    ...
    def update_GUI(self):
        ...
        self.plotButton.setEnabled(has_exp or has_ref)
        self.saveResultsButton.setEnabled(has_exp)
        self.calibrationGroupBox.setEnabled(has_exp)
        self.manualCorrectionGroupBox.setEnabled(has_both)
        self.autoCorrectionGroupBox.setEnabled(has_both)

        const_heat_exch = self.autoOption2Selector.currentText() == "heat exchange constant value"
        self.heatExchLabel.setEnabled(const_heat_exch and has_both)
        self.heatExchInput.setEnabled(const_heat_exch and has_both)
        self.calculateAutoButton.setEnabled(has_both)
        self.calculateManualButton.setEnabled(has_both)
        self.displayRateButton.setEnabled(has_both)
    ...
    def update_plot_after_add_remove(self):
        ...
        if empty_paths:
            for path in empty_paths:
                data = self._load_fast_heat_data_group(path)
